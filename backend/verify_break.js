...
try {
    ...
    // 5. Manually simulate the break start logic
    ...
    const updatedTask = db.prepare("SELECT status FROM tasks WHERE id = ?").get(task.id);
    const updatedWorker = db.prepare("SELECT is_on_break, status FROM users WHERE id = ?").get(worker.id);

    console.log('Task status:', updatedTask.status);
    console.log('Worker is_on_break:', updatedWorker.is_on_break);
    console.log('Worker status:', updatedWorker.status);

    if (updatedTask.status === 'paused' && updatedWorker.is_on_break === 1 && updatedWorker.status === 'paused') {
        console.log('✅ Logic Verified: Active tasks are paused when break starts.');
    } else {
